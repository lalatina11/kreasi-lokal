import AcceptOrderForm from "@/components/forms/AcceptOrderForm";
import AddShippingNumberForm from "@/components/forms/AddShippingNumberForm";
...
export const getOrdersForMerchant = createServerFn({ method: "GET" })
  .inputValidator(z.string())
  .handler(async ({ data: merchantId }) => {
      ...
      where: (order, { exists, eq }) => {
        return exists(
          db
            ...
            .where(
              and(
                eq(tables.orderItem.orderId, order.id),
                eq(tables.orderItem.merchantId, merchantId)
              )
            )
        );
      },
...
export const Route = createFileRoute("/dashboard/merchant/orders/")({
  ...
  loader: async () => {
    ...
    const orders = await getOrdersForMerchant({ data: session.user.id });
    return { orders };
  },
});
...
function RouteComponent() {
  ...
  const isOrderAwaitingShipmentNumber = (status: string) => {
    return status === "awaiting_shipment_number";
  };
                ...
                {order.items.map((item) => (
                  <div key={item.id} className="flex flex-col gap-1 text-sm">
                    ...
                    {isOrderPending(order.status as string) ? (
                      ...
                      <AddShippingNumberForm orderId={order.id} />
                    ) : (
